...
myKNN = function(traindata, testdata, k){
  ...
  for (i in 1:nrow(testdata)){
    temp = matrix(testdata[i,], nrow = nrow(traindata), ncol = ncol(traindata), byrow = TRUE)
...
```{r}
sum(pred_vec_k1 > 1) #count classified as label 2
sum(pred_vec_k1 < 2) #count classified as label 1
```
...
```{r}
cvKNNAveErrorRate(3, traindata, Ytrain, 10)
```
...
```{r}
cvKNN(traindata, Ytrain, foldNum = 10)
```
...
```{r}
Ytest_pred_Bayes = apply(testdata, 1, mixnorm, centers0 = m0, centers1 = m1, s = sim_params$s)
...
test.err.Bayes = sum(Ytest !=  Ytest_pred_Bayes) / (nrow(testdata))
test.err.Bayes
```
...
* Name your file starting with 
  ...
  <span style="color: red;">Assignment 1 7127 mychen12 MaxChen.html</span>
